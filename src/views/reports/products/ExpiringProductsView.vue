...
const getAllProducts = async () => {
  ...
  try {
    ...
    // Narrow the notification window (14 days) in SQL with a day of slack;
    // the exact boundary is still enforced by the filter below.
    ...
    const data = await db('products')
        ...
        .whereRaw('expiration > ? AND expiration <= ?', [db.fn.now(), cutoffStr])
