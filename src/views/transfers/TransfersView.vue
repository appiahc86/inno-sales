...
const productQuery = () => db('products').select('id', 'quantity', 'warehouseQty', 'productName');

//get initial products shown before the user types anything
const getAllProducts = async () => {
  try {
    loading.value = true;
    products.value = await productQuery().orderBy('productName', 'asc').limit(PRODUCT_RESULT_LIMIT);
  }
...
const onProductSearch = (search, setSearchLoading) => {
  ...
  productSearchDebounce = setTimeout(async () => {
    try {
      ...
      if (search.trim()) query = query.where('productName', 'like', `%${search}%`);
      products.value = await query.orderBy('productName', 'asc').limit(PRODUCT_RESULT_LIMIT);
    }
    catch (e){ ipcRenderer.send('errorMessage', e.message) }
