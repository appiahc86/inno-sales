...
const activeQuantity = ref(null); //quantity threshold of the last executed search
...
const baseQuery = () => db('products')
    ...
    .whereRaw('?? < ?', ['quantity', activeQuantity.value]);
...
const fetchRecords = async () => {
  try {
    ...
    if (totalRecords.value) message.value = `Products with Qty less than ${activeQuantity.value}`
...
const search = async (e) => {
  ...
  loading.value = true;
  try {
    activeQuantity.value = quantity.value;
    lazyParams.first = 0;
    await fetchRecords();
  }
  catch (e){ ipcRenderer.send('errorMessage', e.message) }
...
const exportToPDF = async () => {
  try {
    ...
    await loadPrintOut();
    const printOut = document.querySelector('#printOut');
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Low Stock</title></head><body>${printOut.outerHTML}</body></html>`;

    const result = await ipcRenderer.invoke('exportToPDF', {
      html,
      defaultFileName: `Low Stock ${new Date().toISOString().slice(0, 10)}.pdf`
    });
